...
import static org.apache.hadoop.fs.swifta.util.SwiftTestUtils.assertListStatusFinds;
import static org.apache.hadoop.fs.swifta.util.SwiftTestUtils.cleanup;
import static org.apache.hadoop.fs.swifta.util.SwiftTestUtils.dumpStats;
import static org.apache.hadoop.fs.swifta.util.SwiftTestUtils.touch;
...
public class TestSwiftFileSystemLsOperations extends SwiftFileSystemBaseTest {
  ...
  /**
   * Create subdirectories and files under test/ for those tests that want them. Doing so adds
   * overhead to setup and teardown, so should only be done for those tests that need them.
   * 
   * @throws IOException on an IO problem
   */
  private void createTestSubdirs() throws IOException {
    testDirs =
        new Path[] {path("/test/hadoop/a"), path("/test/hadoop/b"), path("/test/hadoop/c/1"),};
